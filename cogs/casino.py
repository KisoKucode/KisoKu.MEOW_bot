...
from config import DAILY_AMOUNT # Assuming DAILY_AMOUNT is in config
...
class Casino(commands.Cog):
    ...
    def hand_value(self, hand):
        value = sum(self.ranks[card['rank']] for card in hand)
    ...
    class BlackjackView(ui.View):
        def __init__(self, *, interaction: discord.Interaction, apuesta: int, casino_cog):
            super().__init__(timeout=180.0)
            self.interaction = interaction
            self.apuesta = apuesta
            self.casino_cog = casino_cog # Reference to the Casino cog
            self.deck = self.casino_cog.create_deck()
        ...
        def build_embed(self, game_over=False):
            player_value = self.casino_cog.hand_value(self.player_hand)
            dealer_value = self.casino_cog.hand_value(self.dealer_hand)
            
            embed = Embed(title="🃏 Blackjack 🃏", color=discord.Color.dark_blue())
            embed.add_field(name=f"Tu mano ({player_value})", value=self.casino_cog.hand_to_string(self.player_hand), inline=False)
            
            if game_over:
                embed.add_field(name=f"Mano del crupier ({dealer_value})", value=self.casino_cog.hand_to_string(self.dealer_hand), inline=False)
            else:
                embed.add_field(name=f"Mano del crupier ({self.casino_cog.ranks[self.dealer_hand[0]['rank']]})", value=f"{self.casino_cog.hand_to_string([self.dealer_hand[0]])} `?`", inline=False)
        ...
        async def stand(self, interaction: discord.Interaction, button: ui.Button):
            ...
            while self.casino_cog.hand_value(self.dealer_hand) < 17:
    ...
    def create_poker_deck(self):
        deck = [{'rank': rank, 'suit': suit} for rank in self.poker_ranks for suit in self.suits]
        random.shuffle(deck)
        return deck
    ...
    def get_hand_rank(self, hand):
        ...
        poker_ranks_in_hand = sorted([self.poker_ranks[card['rank']] for card in hand])
    ...
    class DrawButton(ui.Button):
        ...
        async def callback(self, interaction: discord.Interaction):
            ...
            hand_rank_name = self.casino_cog.get_hand_rank(final_hand)
            winnings = 0
            if hand_rank_name in self.casino_cog.VIDEO_POKER_PAYOUTS:
    ...
    class VideoPokerView(ui.View):
        def __init__(self, *, interaction: discord.Interaction, apuesta: int, deck: list, hand: list, casino_cog):
            super().__init__(timeout=180.0)
            self.interaction = interaction
            self.apuesta = apuesta
            self.deck = deck
            ...
            for i in range(5):
                self.add_item(self.casino_cog.CardButton(i, hand[i]))
            
            self.add_item(self.casino_cog.DrawButton(casino_cog=self.casino_cog))
    ...
    async def poker(self, interaction: discord.Interaction, apuesta: int):
        ...
        deck = self.create_poker_deck()
        hand = [deck.pop() for _ in range(5)]

        view = self.VideoPokerView(interaction=interaction, apuesta=apuesta, deck=deck, hand=hand, casino_cog=self)
        
        embed = Embed(title="🃏 Poker 🃏", description="Elige las cartas que quieres conservar y pulsa 'Repartir'.", color=discord.Color.purple())
        embed.add_field(name="Tu Mano", value=self.hand_to_string(hand), inline=False)
        
        payout_table = "\n".join([f"**{name}**: {payout}x" for name, payout in self.VIDEO_POKER_PAYOUTS.items()])
    ...
    @app_commands.command(name="dados_apuesta", description="Apuesta si la suma de dos dados será alta, baja o 7.")
    @app_commands.choices(eleccion=[
    ...
    async def play_roulette(self, interaction: discord.Interaction, apuesta: int, did_win_check: callable, prize_multiplier: int, bet_type: str):
        ...
        winning_color_emoji = self.ROULETTE_NUMBERS[winning_number]
    ...
    async def ruleta_numero(self, interaction: discord.Interaction, apuesta: int, numero: app_commands.Range[int, 0, 36]):
        await self.play_roulette(
            interaction, apuesta,
    ...
    async def ruleta_color(self, interaction: discord.Interaction, apuesta: int, color: app_commands.Choice[str]):
        color_emoji = '🔴' if color.value == "rojo" else '⚫'
        await self.play_roulette(
            interaction, apuesta,
            did_win_check=lambda res: res != 0 and self.ROULETTE_NUMBERS[res] == color_emoji,
            prize_multiplier=1,
            bet_type=color.name
        )
    ...
    async def ruleta_paridad(self, interaction: discord.Interaction, apuesta: int, paridad: app_commands.Choice[str]):
        await self.play_roulette(
            interaction, apuesta,
    ...
    async def carrera(self, interaction: discord.Interaction, apuesta: int, corredor: app_commands.Range[int, 1, 5]):
        ...
        embed.set_image(url="https://images-ext-1.discordapp.net/external/f9phbx09HyO_qkqxK8jELA4HWIM26WoIMm82ErlloMg/https/media.giphy.com/media/v1.Y2lkPTc5MGI3NjExbjltNXE1MzNtZXV3Mmx2NTk3cHF2dzczY2xrdjhmMzU2OXBzMWh1ZCZlcD12MV9naWZzX3NlYXJjaCZjdD1n/5Wi5Ccpvv0dJQ4BE3p/giphy.gif?width=600&height=338")
